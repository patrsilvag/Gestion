...
export default function Proveedores() {
  const storageKey = "mant_proveedores";
  const [data, setData] = useState<any[]>([]);
  const [editingIndex, setEditingIndex] = useState<number|null>(null);
  const [form, setForm] = useState<any>({ 'rut': '', 'razon': '', 'familia': '', 'contacto': '', 'correo': '', 'estado': '' });
  ...
  const resetForm = ()=>setForm({ 'rut': '', 'razon': '', 'familia': '', 'contacto': '', 'correo': '', 'estado': '' });
  ...
  return (
    <div className="space-y-4">
      <Card>
        <CardHeader><CardTitle>Proveedores — Formulario</CardTitle></CardHeader>
...
<div className="flex flex-col gap-1">
  ...
  <Input type="text" value={form['rut']} onChange={(e)=>setForm((s: any) =>({...s, 'rut': e.target.value}))} />
</div>
...
<SelectItem value="inactivo">Inactivo</SelectItem>
      ...
      <Card>
        <CardHeader><CardTitle>Proveedores — Listado</CardTitle></CardHeader>
