...
export default function TiposDocumento() {
  const storageKey = "mant_tiposdocumento";
  const [data, setData] = useState<any[]>([]);
  const [editingIndex, setEditingIndex] = useState<number|null>(null);
  const [form, setForm] = useState<any>({ 'codigo': '', 'tipo': '', 'extensiones': '', 'peso': '', 'obligatorio': '', 'estado': '' });
  ...
  const resetForm = ()=>setForm({ 'codigo': '', 'tipo': '', 'extensiones': '', 'peso': '', 'obligatorio': '', 'estado': '' });
  ...
  return (
    <div className="space-y-4">
      <Card>
        <CardHeader><CardTitle>TiposDocumento — Formulario</CardTitle></CardHeader>
...
<SelectItem value="inactivo">Inactivo</SelectItem>
      ...
      <Card>
        <CardHeader><CardTitle>TiposDocumento — Listado</CardTitle></CardHeader>
...
<TableCell>{row['extensiones']}</TableCell>
<TableCell>{row['peso']}</TableCell>
<TableCell>{row['obligatorio']}</TableCell>
